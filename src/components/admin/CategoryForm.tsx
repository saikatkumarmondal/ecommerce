...
interface CategoryFormProps {
  open: boolean;
  onClose: () => void;
  editingCategory?: { id: string; name: string; image?: string } | null;
}

export function CategoryForm({ open, onClose, editingCategory }: CategoryFormProps) {
  ...
  useEffect(() => {
    if (editingCategory) {
      setForm({ name: editingCategory.name, image: editingCategory.image ?? "" });
    } else {
      setForm({ name: "", image: "" });
    }
    setError("");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCategory) {
        await updateCategory({ id: editingCategory.id, data: form }).unwrap();
      } else {
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  {editingCategory ? "Update structural cluster values" : "Establish a root data category item"}
                </p>
              </div>
              <motion.button
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {/* Dynamic Image Graphic Preview Wrapper */}
              {form.image && !previewError && (
                <motion.div
                  initial={{ opacity: 0, y: 6 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="relative group w-full h-36 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm bg-slate-50 dark:bg-slate-950"
                >
                  <img
                    src={form.image}
              ...
              {/* Image Load Failure Safe Fallback */}
              {form.image && previewError && (
                <div className="w-full h-24 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-800 flex flex-col items-center justify-center gap-1.5 bg-slate-50/50 dark:bg-slate-950/50 text-slate-400">
                  <ImageIcon className="w-5 h-5 stroke-[1.5]" />
                  <span className="text-xs">Image resources failed to load</span>
                </div>
              )}
